...
import {
  ...
  GoogleGenAI,
  MediaResolution,
} from "@google/genai"; // Using the new @google/genai package (migrated from deprecated @google/generative-ai)
...
async function processVideoAnalysis(
  ...
  try {
    ...
    await db
      .update(schema.videoAnalysisJobs)
      .set({
        status: "processing",
        started_at: new Date(),
      })
    ...
    const params: GenerateContentParameters = {
      ...
      config: {
        responseMimeType: "text/plain",
        mediaResolution: job.use_low_resolution
    ...
    if (!analysisResult.trim()) {
      throw new Error(
        "No analysis could be generated for this video. The video might be private, age-restricted, or unavailable."
      );
    }

    // Calculate processing time
    const processingTime = job.started_at
      ? Math.floor((Date.now() - job.started_at.getTime()) / 1000)
      : 0;
  ...
  server.tool(
    "analyze_youtube_video",
    {
      ...
      model: z
        ...
        .describe("AI model to use for analysis"),
      force_low_resolution: z
        .boolean()
        .default(false)
        .describe("Force low resolution processing"),
    },
    async ({ video_url, question, model, force_low_resolution }) => {
      try {
        ...
        const [newJob] = await db
          .insert(schema.videoAnalysisJobs)
          .values({
              ...
              text: JSON.stringify(
                {
                  ...
                  message:
                    "Video analysis job created and queued for processing",
                },
                null,
                2
              ),
        ...
        return {
          content: [
            {
              type: "text",
              text: `Error creating analysis job: ${
  ...
  server.tool(
    ...
    async ({ status, limit }) => {
      try {
        ...
        if (status) {
          query = query.where(
            eq(schema.videoAnalysisJobs.status, status)
          ) as any;
        }
        ...
        const jobSummaries = jobs.map((job: any) => ({
          ...
          youtube_url: job.youtube_url,
          question:
            job.question.substring(0, 100) +
            (job.question.length > 100 ? "..." : ""),
          model: job.model,
          created_at: job.created_at,
          completed_at: job.completed_at,
          processing_time: job.processing_time,
  ...
  return c.json(
    createOpenAPISpec(app, {
      info: {
        ...
        version: "2.0.0",
        description:
          "Asynchronous YouTube video analysis with AI-powered insights",
      },
    })
  );
});
